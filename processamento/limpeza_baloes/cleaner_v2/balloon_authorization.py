...
CONF=0.25
IOU=0.45
ALGORITHM="textoff_level1_balloon_gate_v1"
...
def apply_balloon_authorization(source_images, output_dir, report_path, *, progress_job=None, chapter_name=None):
    ...
    for i,src in enumerate(map(Path,source_images),1):
        ...
        if result.masks is not None:
            for poly in result.masks.xy:
                ...
                if len(pts)>=3:
                    cv2.fillPoly(balloon_mask,[pts],255); balloons+=1
    ...
    report={"schema_version":1,"algorithm":ALGORITHM,"policy":"cleaner_mask_intersection_balloon_mask",
            "fail_closed":True,"model":{"repo":MODEL_REPO,"file":MODEL_FILE,"revision":MODEL_REVISION,
