(() => {
  ...
  const icons={
    ...
    export:'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v12m0 0l-4-4m4 4l4-4M5 19h14"/>'
  };
  ...
  function buildText(){return `<div class="fm-mega-grid">${col('Fonte',icons.text,item('clean','Original','Imagens originais',icons.text)+item('clean_merged','Merged','Imagens mescladas',icons.merge))}${col('Resultados',icons.eye,item('textoff_compare','Comparar resultados','Original × imagem limpa',icons.eye)+item('textoff_level3','Correção assistida','Tratar imagens sinalizadas',icons.edit)+item('textoff_special','Especiais','Tratamentos para casos específicos',icons.text))}${col('Acesso',icons.text,'<div class="fm-mega-note">A lógica do Texto Off não foi alterada.</div>')}</div>`}
  function buildExport(){return `<div class="fm-mega-grid">${col('Exportação',icons.export,item('exportacao','Exportar arquivos','Simular e copiar artefatos oficiais para uma pasta local',icons.export))}${col('Conteúdo',icons.export,'<div class="fm-mega-note">Texto Off — MERGED e PDF_MERGE. O destino pode ser qualquer diretório local.</div>')}${col('Segurança',icons.check,'<div class="fm-mega-note">A exportação só é habilitada após uma simulação válida.</div>')}</div>`}
  const panels={overview:{title:'Visão Geral',sub:'Contexto de trabalho, resumo e validação inicial',icon:icons.overview,build:buildOverview},processamento:{title:'Processamento',sub:'Auto Merge, revisão e Merge Manual',icon:icons.list,build:buildProcess},balanceamento:{title:'Balanceamento',sub:'Validação e novos cortes',icon:icons.scale,build:buildBalance},pdf:{title:'Gerar PDF',sub:'Geração por origem',icon:icons.pdf,build:buildPdf},textooff:{title:'Texto Off',sub:'Limpeza por fonte de imagem',icon:icons.text,build:buildText},exportacao:{title:'Exportar arquivos',sub:'Simular e copiar artefatos oficiais',icon:icons.export,build:buildExport}};
