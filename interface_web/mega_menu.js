(() => {
  ...
  function manualRoutes(){
    ...
    const final=cfg.final||first(['merge_manual_final','manual_merge_final','merge_manual_compose','merge_manual_result','merge_manual_apply'])||pageByText('composicao final','resultado merge manual','aplicar composicao');
    return {validate,cuts,final};
  }
  ...
  function buildProcess(){const m=manualRoutes(),cutsReady=manualCutsReady();return `<div class="fm-mega-grid">${col('Auto Merge',icons.merge,[item('merge','Auto Merge','Processamento inicial',icons.merge),item('merge_level2','AM Nível II','Resultados parciais',icons.merge),item('merge_level3','AM Nível III','Tentativa residual',icons.merge),item('merge_level4','AM Nível IV','Processamento dirigido',icons.merge),item('merge_level5','AM Nível V','Classificação residual',icons.merge)].join(''))}${col('Revisão',icons.check,item('review','Revisão Merge','Tratar casos pendentes',icons.check)+item('review_v2','Revisão Merge V2','Fluxo alternativo',icons.edit))}${col('Merge Manual',icons.scissors,item(m.validate,'Validar','Selecionar faixa pendente',icons.eye,!!m.validate)+item(m.cuts,'Novos Cortes','Definir novos pontos de corte',icons.scissors,!!m.cuts&&cutsReady)+item(m.final,'Composição final','Revisar e concluir o merge manual',icons.check,!!m.final)+((!m.validate||!m.cuts||!m.final)?'<div class="fm-mega-note">Rotas do Merge Manual não foram detectadas integralmente. Nenhuma rota foi inventada.</div>':''))}</div>`}
  ...
  function buildText(){return `<div class="fm-mega-grid">${col('Fonte',icons.text,item('clean','Original','Imagens originais',icons.text)+item('clean_merged','Merged','Imagens mescladas',icons.merge))}${col('Contexto',icons.eye,'<div class="fm-mega-note">A obra ativa é definida em Visão Geral.</div>')}${col('Acesso',icons.text,'<div class="fm-mega-note">A lógica do Texto Off não foi alterada.</div>')}</div>`}
