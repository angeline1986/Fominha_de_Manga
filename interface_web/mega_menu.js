(() => {
  ...
  function buildText(){return `<div class="fm-mega-grid">${col('Fonte',icons.text,item('clean','Original','Imagens originais',icons.text)+item('clean_merged','Merged','Imagens mescladas',icons.merge))}${col('Resultados',icons.eye,item('textoff_compare','Comparar resultados','Original × imagem limpa',icons.eye))}${col('Acesso',icons.text,'<div class="fm-mega-note">A lógica do Texto Off não foi alterada.</div>')}</div>`}
