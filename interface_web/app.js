...
function renderValidateImagesBody(){
  ...
  let list=dimensionFilter==="divergent"?allList.filter(x=>x.dimension?.status==="REQUER_ANALISE"):allList;
  ...
  let exceptions=a?.correction_applied?[]:(a?.images||[]).filter(x=>x.classification==="EXCECAO_DIMENSAO");
  let detail=dimensionStep===1?`<div class="dim-detail"><div class="dim-title"><div><span class="caption">ETAPA 1</span><h2>${current?`Capítulo ${esc(current.chapter)}`:"Selecione um capítulo"}</h2></div>${a?`<span class="dim-status ${a.status==='OK'||a.status==='CORRIGIDO'?'ok':'attention'}">${a.status==='CORRIGIDO'?'Corrigido':a.status==='OK'?'OK':'Requer análise'}</span>`:''}</div>${!a?`<div class="dim-empty">Este capítulo ainda não possui análise de dimensões.</div>`:`<div class="dim-kpis"><div><b>${a.image_count}</b><span>Imagens</span></div><div><b>${a.dominant_width}px</b><span>Largura dominante</span></div><div><b>±${a.tolerance_percent}%</b><span>Tolerância</span></div><div><b>${a.exceptions}</b><span>Exceções</span></div></div><h3>Divergências encontradas</h3><div class="dim-exceptions">${exceptions.map(x=>`<div><b>${esc(x.file)}</b><span>${x.width} × ${x.height}px</span><small>Diferença: ${x.difference_px>0?'+':''}${x.difference_px}px (${x.difference_percent.toFixed(2)}%)</small></div>`).join('')||'<div class="dim-okbox">Nenhuma divergência fora da tolerância.</div>'}</div>${exceptions.length?`<button class="btn primary" onclick="dimSetStep(2)">Gerar sugestão de correção</button>`:''}`}</div>`:
  dimensionStep===2?`<div class="dim-detail"><div class="dim-title"><div><span class="caption">ETAPA 2</span><h2>Capítulo ${esc(current?.chapter||'')}</h2></div><span class="dim-status attention">${exceptions.length} imagem(ns)</span></div><div class="dim-suggestion"><h3>Sugestão de correção</h3><p>Normalizar proporcionalmente somente as imagens fora da tolerância para a largura dominante de <b>${a?.dominant_width||'—'} px</b>. Os arquivos de IMG permanecerão intactos.</p><div class="dim-compare-list">${exceptions.map(x=>{let h=Math.max(1,Math.round(x.height*((a?.dominant_width||x.width)/x.width)));let ratio=Math.min(1,(a?.dominant_width||x.width)/x.width);let src=dimensionSourceUrl(current?.chapter||'',x.file);return `<section class="dim-compare"><header><b>${esc(x.file)}</b><span>Normalização proporcional</span></header><div class="dim-before-after"><div class="dim-preview-card"><div class="dim-preview-head"><strong>Antes</strong><span>${x.width} × ${x.height}px</span></div><div class="dim-preview-stage before"><img src="${src}" alt="Antes · ${esc(x.file)}"></div></div><div class="dim-preview-arrow">→</div><div class="dim-preview-card"><div class="dim-preview-head"><strong>Depois</strong><span>${a?.dominant_width} × ${h}px</span></div><div class="dim-preview-stage after"><img src="${src}" alt="Depois · ${esc(x.file)}" style="width:${(ratio*100).toFixed(2)}%"></div></div></div><div class="dim-fix"><div><b>Original preservado</b><span>${x.width} × ${x.height}px</span></div><strong>→</strong><div><b>Correção sugerida</b><span>${a?.dominant_width} × ${h}px</span></div></div></section>`}).join('')}</div><div class="dim-note">A prévia “Depois” representa a escala proporcional proposta. A correção real só é materializada após clicar em “Aplicar correções”. Nenhuma imagem original será sobrescrita.</div></div><div class="dim-actions"><button class="btn" onclick="dimSetStep(1)">Voltar</button><button class="btn primary" onclick="applyDimensionCorrection('${esc(current?.chapter||'')}')">Aplicar correções</button></div></div>`:
  `<div class="dim-detail"><div class="dim-title"><div><span class="caption">ETAPA 3</span><h2>Composição final · Capítulo ${esc(current?.chapter||'')}</h2></div><span class="dim-status ok">Concluído</span></div><div class="dim-final"><h3>Resultado</h3><p>As imagens corrigidas são materializadas separadamente e a composição final preserva a ordem e os nomes das imagens originais.</p><div class="dim-kpis"><div><b>${a?.image_count||0}</b><span>Analisadas</span></div><div><b>${Math.max(0,(a?.image_count||0)-(a?.exceptions||0))}</b><span>Sem alteração</span></div><div><b>${a?.corrected_count??a?.source_exceptions??a?.exceptions??0}</b><span>Corrigidas</span></div><div><b>0</b><span>Originais alteradas</span></div></div><div class="dim-note">IMG original preservado. Resultado em FLUXO_SECUNDARIO/ANALISE_DIMENSOES/COMPOSICAO_FINAL.</div><button class="btn" onclick="openDimensionFolder()">Abrir pasta</button></div></div>`;
  host.innerHTML=dimensionFlow()+`<div class="dim-toolbar"><input class="search" placeholder="Buscar capítulo..." oninput="document.querySelectorAll('.dim-row').forEach(r=>r.hidden=!r.dataset.n.includes(this.value.toLowerCase()))"><div class="dim-filter-tabs" role="group" aria-label="Filtrar capítulos"><button class="${dimensionFilter==='all'?'active':''}" onclick="dimSetFilter('all')">Todos</button><button class="${dimensionFilter==='divergent'?'active':''}" onclick="dimSetFilter('divergent')">Com divergência <span>${divergentCount}</span></button></div><label class="dim-tolerance">Tolerância <input type="number" min="0" max="20" step="0.5" value="${dimensionTolerance}" onchange="dimensionTolerance=Number(this.value)||3"> %</label><button class="btn" onclick="document.querySelectorAll('.dim-row:not([hidden]) .dim-ck:not(:disabled)').forEach(x=>x.checked=true)">Selecionar visíveis</button><button class="btn primary" onclick="analyzeDimensions()">Analisar selecionadas</button></div><div class="dim-layout"><div class="dim-list">${list.map(x=>`<div class="dim-row ${String(x.chapter)===String(dimensionChapter)?'active':''}" data-n="${esc(String(x.chapter).toLowerCase())}"><input class="dim-ck" type="checkbox" value="${esc(x.chapter)}"><button onclick="dimSelectChapter('${esc(x.chapter)}')"><b>${esc(x.chapter)}</b><span class="${x.dimension?.status==='OK'||x.dimension?.status==='CORRIGIDO'?'ok':x.dimension?.status==='REQUER_ANALISE'?'attention':'muted'}">${x.dimension?.status==='CORRIGIDO'?`✓ Corrigido (${x.dimension.corrected_count||0})`:x.dimension?.status==='OK'?'✓ OK':x.dimension?.status==='REQUER_ANALISE'?`! ${x.dimension.exceptions} exceção(ões)`:'Não analisado'}</span></button></div>`).join('')||'<div class="dim-filter-empty">Nenhum capítulo com divergência.</div>'}</div>${detail}</div>`;
}
...
async function applyDimensionCorrection(ch){if(!ch)return;if(!(await askAppModal("Aplicar correção",`Normalizar proporcionalmente as exceções do Cap. ${ch}, preservando IMG?`,"Aplicar")))return;dimensionStep=3;job("dimension_correct",[ch],{tolerance:dimensionTolerance});setTimeout(loadDimensionAnalysis,1200);renderValidateImagesBody()}
