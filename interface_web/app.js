...
function updateSidebarSyncState(){
  const el=document.querySelector("#sidebarUpdatedAt .sidebar-updated-label");
  if(!el)return;
  el.textContent=lastUpdated?`Atualizado às ${lastUpdated}`:"Ainda não sincronizado";
}
...
let dimensionData=null,dimensionChapter=null,dimensionStep=1,dimensionTolerance=3,dimensionFilter="all",dimensionQuery="";
...
function dimSetFilter(filter){dimensionFilter=filter;tablePage=1;renderValidateImagesBody()}
...
function renderValidateImagesBody(){
  ...
  let list=dimensionFilter==="divergent"
    ...
    :dimensionFilter==="ok"
      ...
      :dimensionFilter==="pending"
        ...
        :allList;
  let q=String(dimensionQuery||"").trim().toLowerCase();
  if(q)list=list.filter(x=>String(x.chapter).toLowerCase().includes(q));
  let pages=Math.max(1,Math.ceil(list.length/PAGE_SIZE));
  tablePage=Math.min(Math.max(1,tablePage),pages);
  let rows=list.slice((tablePage-1)*PAGE_SIZE,tablePage*PAGE_SIZE);
  if(dimensionChapter&&!list.some(x=>String(x.chapter)===String(dimensionChapter)))dimensionChapter=null;
  ...
  let detail=dimensionStep===1?(a?.status==="REQUER_ANALISE"?`<div class="dim-detail"><div class="dim-title"><div><h2>Divergências · Capítulo ${esc(current?.chapter||'')}</h2></div><span class="dim-status attention">Requer análise</span></div><div class="dim-exceptions">${exceptions.map(x=>`<div><b>${esc(x.file)}</b><span>${x.width} × ${x.height}px</span><small>Diferença: ${x.difference_px>0?'+':''}${x.difference_px}px (${x.difference_percent.toFixed(2)}%)</small></div>`).join('')}</div>${exceptions.length?`<button class="btn primary" onclick="dimSetStep(2)">Sugestão de correção</button>`:''}</div>`:""):
  ...
  let pager=`<div class="table-pager"><span>${list.length?((tablePage-1)*PAGE_SIZE+1):0}-${Math.min(tablePage*PAGE_SIZE,list.length)} de ${list.length}</span><div><button class="btn" ${tablePage<=1?"disabled":""} onclick="changeTablePage(-1)">&lt;&lt;</button><span class="page-indicator">${tablePage} / ${pages}</span><button class="btn" ${tablePage>=pages?"disabled":""} onclick="changeTablePage(1)">&gt;&gt;</button></div></div>`;
  let dimRows=rows.map(x=>{
    let d=x.dimension;
    let imageCount=d?.image_count??x.pages??0;
    let width=d?.dominant_width!=null?`${d.dominant_width} px`:"—";
    let variation="—";
    if(d?.images?.length){
      let vals=d.images.map(img=>Number(img.difference_percent)).filter(Number.isFinite);
      if(vals.length){
        let max=vals.reduce((best,v)=>Math.abs(v)>Math.abs(best)?v:best,vals[0]);
        variation=`${max>0?"+":""}${max.toFixed(1).replace(".",",")}%`;
      }
    }
    let exceptionCount=d?.exceptions??0;
    let status=d?.status==="OK"
      ?'<span class="dim-table-status ok">OK</span>'
      :d?.status==="REQUER_ANALISE"
        ?'<span class="dim-table-status attention">Revisar</span>'
        :'<span class="dim-table-status muted">Não analisado</span>';
    return `<tr class="${String(x.chapter)===String(dimensionChapter)?'dim-table-active':''}" data-n="${esc(String(x.chapter).toLowerCase())}"><td><input class="dim-ck" type="checkbox" value="${esc(x.chapter)}" onchange="syncVisibleMaster(document.querySelector('.dim-visible-master'),'.dim-ck')"></td><td><button class="dim-chapter-link" onclick="dimSelectChapter('${esc(x.chapter)}')">${esc(x.chapter)}</button></td><td>${imageCount}</td><td>${width}</td><td>${variation}</td><td>${exceptionCount}</td><td>${status}</td></tr>`;
  }).join("");
  host.innerHTML=dimensionFlow()+`<div class="toolbar standard-filterbar"><input class="search" placeholder="Buscar capítulo..." value="${esc(dimensionQuery)}" oninput="dimensionQuery=this.value;tablePage=1;renderValidateImagesBody()"><div class="status-filter" role="group" aria-label="Filtrar capítulos"><button class="tab ${dimensionFilter==='all'?'active':''}" onclick="dimSetFilter('all')">Todos</button><button class="tab ${dimensionFilter==='ok'?'active':''}" onclick="dimSetFilter('ok')">OK <span>${okCount}</span></button><button class="tab ${dimensionFilter==='divergent'?'active':''}" onclick="dimSetFilter('divergent')">Com exceções <span>${divergentCount}</span></button><button class="tab ${dimensionFilter==='pending'?'active':''}" onclick="dimSetFilter('pending')">Não analisados <span>${pendingCount}</span></button></div><label class="dim-tolerance">Tolerância <input type="number" min="0" max="20" step="0.5" value="${dimensionTolerance}" onchange="dimensionTolerance=Number(this.value)||3"> %</label><button class="btn primary filter-primary-action" onclick="analyzeDimensions()">Analisar</button></div><div class="panel dim-table-panel"><table class="dim-table"><thead><tr><th>${visibleMaster(".dim-ck","dim-visible-master")}</th><th>CAP.</th><th>IMAGENS</th><th>LARGURA DOMINANTE</th><th>VARIAÇÃO</th><th>EXCEÇÕES</th><th>STATUS</th></tr></thead><tbody>${dimRows||'<tr><td colspan="7" class="dim-table-empty">Nenhum capítulo encontrado.</td></tr>'}</tbody></table>${pager}</div><div class="dim-detail-wrap">${detail}</div>`;
}
async function analyzeDimensions(){let ch=dimSelected();if(!ch.length)return toast("Selecione ao menos um capítulo.");dimensionChapter=String(ch[0]);dimensionStep=1;job("dimension_analyze",ch,{tolerance:dimensionTolerance});setTimeout(loadDimensionAnalysis,1200)}
  ...
  let body=rows.map(x=>{
    ...
    let result=pending
      ...
      :(!d.valid
        ...
        :(res
          ?`<span class="bad">Sem composição completa SAFE</span> · <button class="l3-result-link" onclick="reviewCh='${esc(x.chapter)}';page='merge_level5';document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.page==='merge_level5'));render()">Auto-Merge Nível V</button>`
...
function mergeLevel5(r){
  let list=data.chapters.filter(x=>x.merge_level5_pending||x.merge_level5_detail?.available);
  let filtered=list.filter(x=>!window._tableQuery||String(x.chapter).toLowerCase().includes(String(window._tableQuery).toLowerCase()));
  if(tableStatus==="pending")filtered=filtered.filter(x=>!!x.merge_level5_pending);
  if(tableStatus==="done")filtered=filtered.filter(x=>!x.merge_level5_pending&&x.merge_level5_detail?.available);
...
const JOB_LABELS={
  ...
  merge_level5:"Auto-Merge Nível V",
...
function mergeOperationResultModal(j,s){
  ...
  if(!["merge","merge_level2","merge_level3","merge_level4","merge_level5"].includes(action)) return false;
