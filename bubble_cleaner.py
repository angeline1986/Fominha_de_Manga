...
def resolve_model(model_path: Path | None = None) -> Path:
    ...
    cache_dir = Path(__file__).resolve().parent / ".cache" / "bubble_cleaner"
    cache_dir.mkdir(parents=True, exist_ok=True)
    cached = cache_dir / Path(HF_MODEL_FILE).name

    if cached.is_file():
        digest = sha256_file(cached)
        if digest == MODEL_SHA256:
            print(f"[MODELO] Cache local validado: {cached}")
            return cached
        print("[MODELO] Cache local inválido; obtendo novamente...")
    ...
    if sha256_file(cached) != MODEL_SHA256:
        cached.unlink(missing_ok=True)
        raise RuntimeError("Falha ao validar a cópia local do modelo.")

    print("[MODELO] SHA256 validado e cache local criado.")
    return cached
...
def detect_regions(rgb: np.ndarray, model_path: Path, conf: float = 0.55) -> list[Detection]:
    ...
    for idx, box in enumerate(boxes, start=1):
        ...
        sfx_like_low_texture = brightness >= 225 and texture < 50
        gate_ok = (
            confidence >= conf
            and roi.size > 0
...
def process_image(
    ...
    languages: list[str],
    detector_conf: float,
    ocr_backend: EasyOCRBackend | None = None,
) -> dict:
    source = Path(source)
    ...
    ocr = ocr_backend if ocr_backend is not None else EasyOCRBackend(languages)
    ...
    payload = {
        "schema_version": 35,
        "experiment": "bubble_cleaner_v3_5_official",
        "source": str(source),
        "model": {
            "repo": HF_REPO,
            "file": HF_MODEL_FILE,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bubble Cleaner Experimental V3.3")
